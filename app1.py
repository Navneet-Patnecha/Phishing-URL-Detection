...
model = pickle.load(open("phishing_detection_model.pkl", "rb"))

# Feature extraction class
# ... (include the FeatureExtraction class here)
import ipaddress
import re
import urllib.request
from bs4 import BeautifulSoup
...
class FeatureExtraction:
    ...
    # 4. Symbol@
    def symbol(self):
        if re.findall("@", self.urlparse.netloc):
            return -1
        return 1
...
# ...
    ...
    def AbnormalURL(self):
        try:
            if str(self.response.text) == str(self.whois_response):
                return 1
            else:
                return -1
        except:
...
if __name__ == '__main__':
    ...
    port = random.randint(2000, 9000)  # Generate a random port number between 2000 and 9000
    host = '127.0.0.1'
    ...
    while True:
        try:
            ...
            port = random.randint(2000, 9000)  # Generate a new random port if the current one is in use
